...
let player1 = {current:0, target:0, x:0, y:0, colour:'#8a2d', id:'You'};
let player2 = {current:0, target:0, x:0, y:0, colour:'#f36d', id:'AI'};
...
const rollDice = (evt) => {
	...
	message.innerHTML = activePlayer === player1 ? "Rolling..." : 'AI rolling...';
...
const drawObstacles = () => {
	...
	for(let i=0; i < obstacles.length; i++){
		let ob = obstacles[i];
		ctx.strokeStyle = ob.type === 'snake' ? '#0d0' : '#d00';
		ctx.beginPath();
